...
from ciel_omega.memory.monolith.defaults import (
    gamma_from_categories,
    spectral_categories,
    validate_timestamp,
)
from ciel_omega.memory.monolith.rules_engine import RuleHeuristicsEngine
...
class TMPKernel:
    ...
    def second_analysis(self, D: DataVector, D_TYPE: str, D_ATTR: Dict[str, Any]) -> Dict[str, Any]:
        ...
        meta = D.D_M or {}
        hold_threshold = float(meta.get("tmp_hold_threshold", 0.40))
        pass_threshold = float(meta.get("tmp_pass_threshold", 0.70))
        if hold_threshold < 0.0:
            hold_threshold = 0.0
        if pass_threshold > 1.0:
            pass_threshold = 1.0
        if hold_threshold > pass_threshold:
            hold_threshold = pass_threshold

        if W_total < hold_threshold:
            return {"verdict": "REJECT", "reason": "Too weak.", "weights": Wp, "gamma": gamma, "categories": cat, "bifurcation": 0, "thresholds": {"hold": hold_threshold, "pass": pass_threshold}}
        if W_total < pass_threshold:
            return {"verdict": "HOLD", "reason": "Needs clarification.", "weights": Wp, "gamma": gamma, "categories": cat, "bifurcation": 0, "thresholds": {"hold": hold_threshold, "pass": pass_threshold}}
        bifurcation = 1
        if meta.get("require_trusted_source_for_promotion") and not meta.get("trusted_source"):
            bifurcation = 0
        if not meta.get("durable_write_allowed", True):
            bifurcation = 0
        return {"verdict": "PASS", "weights": Wp, "gamma": gamma, "categories": cat, "bifurcation": bifurcation, "thresholds": {"hold": hold_threshold, "pass": pass_threshold}}
