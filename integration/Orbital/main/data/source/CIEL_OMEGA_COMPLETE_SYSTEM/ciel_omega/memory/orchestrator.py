...
from .orbital_holonomy import compute_consolidation_holonomy
...
class HolonomicMemoryOrchestrator:
    ...
    def _update_semantic(self, episode: Episode, current_time: float, events: ConsolidationEvents, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        if candidate:
            ...
            if candidate.is_mature(
                ...
                gate = self._consolidation_gate("semantic", episode, metadata)
                self.m8.log_decision(f"Semantic holonomy gate: {key}", metadata={"channel": "M3", "allowed": gate["allowed"], "holonomy_quality": gate["holonomy_quality"], "threshold": gate["threshold"], "loop_coherent": gate["loop_coherent"]})
                if gate["allowed"]:
    ...
    def _update_procedural(self, episode: Episode, current_time: float, events: ConsolidationEvents, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        if candidate:
            ...
            if candidate.is_mature(
                ...
                gate = self._consolidation_gate("procedural", episode, metadata)
                self.m8.log_decision(f"Procedural holonomy gate: {key}", metadata={"channel": "M4", "allowed": gate["allowed"], "holonomy_quality": gate["holonomy_quality"], "threshold": gate["threshold"], "loop_coherent": gate["loop_coherent"]})
                if gate["allowed"]:
    ...
    def _update_affective(self, episode: Episode, current_time: float, events: ConsolidationEvents, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        if candidate:
            ...
            if candidate.is_mature(
                ...
                gate = self._consolidation_gate("affective", episode, metadata)
                self.m8.log_decision(f"Affective holonomy gate: {key}", metadata={"channel": "M5", "allowed": gate["allowed"], "holonomy_quality": gate["holonomy_quality"], "threshold": gate["threshold"], "loop_coherent": gate["loop_coherent"]})
                if gate["allowed"]:
    ...
    def _consolidation_gate(self, channel: str, episode: Episode, metadata: Dict[str, Any]) -> Dict[str, Any]:
        governor = metadata.get("memory_governor") if isinstance(metadata.get("memory_governor"), dict) else {}
        orbital_state = governor.get("orbital_state") if isinstance(governor.get("orbital_state"), dict) else {}
        metrics = compute_consolidation_holonomy(channel, entry_phase=float(episode.phase_at_storage), holder=self, orbital=orbital_state)
        if not governor:
            return {
                "allowed": True,
                "threshold": 0.0,
        ...
        channel_thresholds = governor.get("channel_thresholds") if isinstance(governor.get("channel_thresholds"), dict) else {}
        threshold = float(channel_thresholds.get(channel, governor.get("consolidation_gate_threshold", 0.60)) or 0.60)
        require_loop_coherence = bool(governor.get("require_loop_coherence", False))
        allowed_channels = governor.get("allowed_consolidations") if isinstance(governor.get("allowed_consolidations"), list) else []
        blocked_channels = set(governor.get("blocked_consolidations") or [])
        allowed = channel not in blocked_channels
        if allowed_channels:
            allowed = allowed and channel in allowed_channels
        if require_loop_coherence and not bool(metrics.get("loop_coherent", False)):
            allowed = False
        if float(metrics.get("holonomy_quality", 0.0) or 0.0) < threshold:
            allowed = False
        return {
    ...
    def process_input(self, content: Any, metadata: Optional[Dict[str, Any]] = None, dt: float = 0.1) -> OrchestratorCycleResult:
        ...
        semantic_key = self._update_semantic(episode, current_time, events, metadata)
        procedural_key = self._update_procedural(episode, current_time, events, metadata)
        affective_key = self._update_affective(episode, current_time, events, metadata)
