...
from ciel_omega.memory.monolith.data_types import DataVector, MemoriseD, utc_now_iso
...
from ciel_omega.memory.monolith.tsm_storage import TSMWriterSQL
from ciel_omega.memory.monolith.wpm_storage import WPMWriterHDF5
...
class UnifiedMemoryOrchestrator:
    ...
    def _make_memorised(self, D: DataVector, a1: Dict, a2: Dict, rationale: str, source: str = "TMP") -> MemoriseD:
        return MemoriseD(
            memorise_id=str(uuid.uuid4()), created_at=utc_now_iso(),
    ...
    def promote_if_bifurcated(self, D: DataVector, tmp_out: Dict, wave_arrays=None, wave_attrs=None, runtime_policy: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, str]]:
        a2, a1 = tmp_out["OUT"], tmp_out.get("A1", {})
        policy = runtime_policy or {}
        if a2.get("verdict") != "PASS" or a2.get("bifurcation") != 1:
            return None
        if not policy.get("durable_write_allowed", True):
            return None
        if policy.get("require_trusted_source_for_promotion", False) and not bool(D.D_M.get("trusted_source", False)):
            return None
