...
def _build_llama_server_backends() -> Tuple[type[LanguageBackend], type[AuxiliaryBackend]]:
    # Soft import: bundled llama-server may be available even when llama_cpp is absent.
    from .llama_server_backends import LlamaServerAuxBackend, LlamaServerPrimaryBackend

    return LlamaServerPrimaryBackend, LlamaServerAuxBackend
...
def build_gguf_primary_backend(
    *,
    ...
    try:
        LlamaServerPrimaryBackend, _ = _build_llama_server_backends()
        return LlamaServerPrimaryBackend(
            model_path=model_path,
            n_ctx=n_ctx,
            n_threads=n_threads,
            n_gpu_layers=n_gpu_layers,
        ...
        return StubPrimary(name=name, reason=f"llama_cpp unavailable ({type(cpp_error).__name__}: {cpp_error}); llama-server fallback failed ({type(exc).__name__}: {exc})")
...
def build_gguf_aux_backend(
    *,
    ...
    try:
        _, LlamaServerAuxBackend = _build_llama_server_backends()
        return LlamaServerAuxBackend(
            model_path=model_path,
            n_ctx=n_ctx,
            n_threads=n_threads,
            n_gpu_layers=n_gpu_layers,
        ...
        return StubAux(name=name, reason=f"llama_cpp unavailable ({type(cpp_error).__name__}: {cpp_error}); llama-server fallback failed ({type(exc).__name__}: {exc})")
